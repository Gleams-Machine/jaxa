...
def step_impl(context, test_type):
    ...
    response = TestActions.get_test_from_jira_id(
        jaxa_client=context.jaxa.jaxa_client,
        jira_id=test_data.get("test_key")
    )
    test_types = [t.get("testType").get("name") for t in response.get("getTests", {}).get("results", [])]
    assert test_types == [test_type]


@when(u'we search for Tests using JQL {query}')
...
@then(u'Test search results are returned')
...
@when(u'we add a Test to a TestPlan')
def step_impl(context):
    pass
