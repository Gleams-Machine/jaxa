...
class JiraCustomFieldList(MetaCategory):
    def add_customfieldlist_item(self, *, ticket_id: str, item: str, custom_field_id: str) -> dict:
        """
        """
        log.debug(f"Adding [{item}] to CustomField ID {custom_field_id} on ticket {ticket_id}")
        body = {
            "update": {
                custom_field_id: [
                    {
    ...
    def set_customfieldlist(self, *, ticket_id: str, items: list, custom_field_id: str) -> dict:
        """
        """
        log.debug(f"Setting CustomField ID {custom_field_id} on ticket {ticket_id} to {items}")
        body = {
            "fields": {
               custom_field_id: items
           }
        }
        return self._session.request(
            HTTPMethods.PUT, f"rest/api/2/issue/{ticket_id}", json=body
    ...
    def remove_customfieldlist_item(self, *, ticket_id: str, item: str, custom_field_id: str) -> dict:
        """
        """
        log.debug(f"Removing [{item}] from CustomField ID {custom_field_id} on ticket {ticket_id}")
        body = {
            "update": {
                custom_field_id: [
                    {
...
class JiraFields(MetaCategory):
    def get_field_id(self, *, field_name: Optional[str] = None) -> list:
        ...
        if field_name is None:
            return field_list
        matched = [field.get("id") for field in field_list if field.get("name") == field_name]
        return matched
...
class JiraIssues(MetaCategory):
    ...
    def create_issue(self, *, body: Dict):
        ...
        return self._session.request(
            HTTPMethods.POST, "rest/api/2/issue/", json=body
        )
...
class JiraLinks(MetaCategory):
    def get_link_types(self):
        return self._session.request(
            HTTPMethods.GET, f"rest/api/3/issueLinkType"
        )
    ...
    def add_link(self, *, ticket_id: str, outward_issue_id: str, link_type: str) -> dict:
        """
        """
    ...
    def add_jira_link(self, *, ticket_id: str, outward_issue_id: str, link_name: str) -> dict:
        """
        """
        log.debug(f"Creating jira link [{link_name}] from [{ticket_id}] to [{outward_issue_id}]")
