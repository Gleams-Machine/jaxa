...
class Authenticate(MetaCategory):
    ...
    def get_auth_token(self, client_id: str, client_secret: str) -> str:
        ...
        return self._session.request(
            HTTPMethods.POST, "api/v1/authenticate", json=data
        )
...
class GQLTests(MetaCategory):
    ...
    def get_jirakeyid_from_createtest_result(result):
        return result.get("createTest", {}).get("test", {}).get("jira", {}).get("key", "")
    ...
    def get_test_from_jira_id(self, jira_id: str) -> dict:
        ...
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_test_by_jira_id.graphql")
        )
    ...
    def get_tests_by_jql(self, jql_query: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_tests_by_jql.graphql")
        )
    ...
    def create_cucumber_test(self, project_id: str, summary: str, gherkin: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_cucumber_test.graphql")
        )
        variables = dict(
        ...
        log.debug(f"Created Cucumber Test with XRay ID [{test_issue_id}] and Jira ID [{test_jira_id}]")
        return response

    def create_generic_test(self, project_id: str, summary: str, unstructured: str) -> dict:
        """
        Perform graphQL mutation to create a Cucumber test
        :param project_id:
            The Jira project to create the test in
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_generic_test.graphql")
        )
        variables = dict(
            project_id=project_id,
            summary=summary,
        ...
        test_issue_id = self.get_issueid_from_createtest_result(response)
        test_jira_id = self.get_jirakeyid_from_createtest_result(response)
        log.debug(f"Created Generic Test with XRay ID [{test_issue_id}] and Jira ID [{test_jira_id}]")
        return response

    def create_pyautomated_test(self, project_id: str, summary: str, unstructured: str) -> dict:
        """
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_pyautomated_test.graphql")
        )
        variables = dict(
            project_id=project_id,
            summary=summary,
        ...
        test_issue_id = self.get_issueid_from_createtest_result(response)
        test_jira_id = self.get_jirakeyid_from_createtest_result(response)
        log.debug(f"Created pyAutomated Test with XRay ID [{test_issue_id}] and Jira ID [{test_jira_id}]")
        return response
    ...
    def create_manual_test(self, project_id: str, summary: str, steps: list) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_manual_test.graphql")
        )
        variables = dict(
            project_id=project_id,
            summary=summary,
        ...
        test_issue_id = self.get_issueid_from_createtest_result(response)
        test_jira_id = self.get_jirakeyid_from_createtest_result(response)
        log.debug(f"Created Manual Test with XRay ID [{test_issue_id}] and Jira ID [{test_jira_id}]")
        return response
...
class GQLTestPlan(MetaCategory):
    ...
    def get_jirakeyid_from_createtestplan_result(result):
        return result.get("createTestPlan", {}).get("testPlan", {}).get("jira", {}).get("key", "")

    @staticmethod
    def get_test_issueids_from_gettestplan_result(response):
        return [test_result.get("issueId", "") for test_result in response.get("getTestPlan", {}).get("tests", {}).get("results", [])]

    @staticmethod
    def get_test_jiraids_from_gettestplan_result(response):
        return [test_result.get("jira", {}).get("key", "") for test_result in response.get("getTestPlan", {}).get("tests", {}).get("results", [])]

    @staticmethod
    def summarise_testplan_tests(response):
        testplan_id = response.get("getTestPlan", {}).get("issueId", "")
        testplan_jira_id = response.get("getTestPlan", {}).get("jira", {}).get("key", "")
        ...
        for test_result in response.get("getTestPlan", {}).get("tests", {}).get("results", []):
    ...
    def summarise_testplan_execution_runs(response):
        ...
        for testexec_result in response.get("getTestPlan", {}).get("testExecutions", {}).get("results", []):
            for testrun_result in testexec_result.get("testRuns", {}).get("results", []):
                jira_key = testrun_result.get("test", {}).get("jira", {}).get("key", "")
                jira_summary = testrun_result.get("test", {}).get("jira", {}).get("summary", "")
    ...
    def get_testplan_from_id(self, *, testplan_id: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_testplan_from_id.graphql")
        )
    ...
    def get_testplan_by_jql(self, *, jql_query: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_testplan_by_jql.graphql")
        )
        variables = dict(jql_query=jql_query)
        log.debug(f"Executing gql {query=} with {variables=}")
        response = self._session.execute_query(query, variables=variables)
    ...
    def create_test_plan(self, *, project_id: str, summary: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_empty_testplan.graphql")
        )
        variables = dict(
    ...
    def create_test_plan_with_tests(self, *, project_id: str, summary: str, test_ids: str) -> dict:
        """
        Perform graphQL mutation to create a Test Plan
        :param project_id:
            The Jira project to create the test in
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_testplan_with_tests.graphql")
        )
        variables = dict(
            project_key=project_id,
            summary=summary,
    ...
    def assign_tests_to_testplan(self, *, testplan_id: str, test_ids: list) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "assign_tests_to_existing_testplan.graphql")
        )
        variables = dict(
...
class GQLTestExecution(MetaCategory):
    ...
    def get_issueid_from_createtestexecution_result(self, *, result):
        return result.get("createTestExecution", {}).get("testExecution", {}).get("issueId", "")

    def get_jirakeyid_from_createtestexecution_result(self, *, result):
        return result.get("createTestExecution", {}).get("testExecution", {}).get("jira", {}).get("key", "")
    ...
    def get_testexecutions_by_jql(self, *, jql: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_testexecutions_by_jql.graphql")
        )
        variables = dict(
            jql=jql
        )
    ...
    def create_test_execution(self, *, project_id: str, summary: str, testenvs: list, test_ids: list) -> dict:
        """
        Perform graphQL mutation to create a Test Plan
        :param project_id:
            The Jira project to create the test in
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_testexecution.graphql")
        )
        variables = dict(
            project_id=project_id,
            summary=summary,
    ...
    def add_testexecutions_to_testplan(self, *, testplan_id: str, testexecution_ids: list) -> dict:
        """
        Perform graphQL mutation to create a Test Plan
        :param project_id:
            The Jira project to create the test in
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "add_testexecutions_to_testplan.graphql")
        )
        variables = dict(
            testplan_id=testplan_id,
...
class GQLTestRun(MetaCategory):
    ...
    def get_status_from_gettestrunsbyid_result(*, result):
        return [res.get("status", {}).get("name", "") for res in
                result.get("getTestRunsById", {}).get("results", [])]

    def get_test_runs(self, *, testissue_ids, testExecIssueIds):
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_testruns.graphql"))
        variables = dict(
            testIssueIds=testissue_ids,
    ...
    def get_testruns_by_id(self, *, testrun_ids: list):
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_testruns_by_id.graphql"))
        variables = dict(
            testrun_ids=testrun_ids
        )
        log.debug(f"Executing gql {query=} with {variables=}")
        response = self._session.execute_query(query, variables=variables)
        log.debug(f"GQL {response=}")
    ...
    def update_testrun_status(self, *, testrun_id: str, status: str) -> dict:
        ...
        """
        status = status.upper()
        valid_statuses = [status.get("name") for status in GQLTestStatus(self._session).get_teststatuses().get("getStatuses")]
        if status not in valid_statuses:
            # TODO: Custom exception please
            raise Exception(f"Invalid value for TestRun status [{status}]. Valid values {valid_statuses}")
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "update_testrun_status.graphql")
        )
        variables = dict(
    ...
    def update_testrun_timing(self, *, testrun_id: str, started_on: str, finished_on) -> dict:
        """
        Perform graphQL mutation to create a Test Plan
        :param project_id:
            The Jira project to create the test in
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "update_testrun_timing.graphql")
        )
        variables = dict(
            testrun_id=testrun_id,
    ...
    def add_testrun_comment(self, *, testrun_id: str, comment: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "add_testrun_comment.graphql")
        )
        variables = dict(
            testrun_id=testrun_id,
            comment=comment
...
class GQLTestSets(MetaCategory):
    ...
    def get_jiraid_from_createtestset_result(*, result):
        return result.get("createTestSet", {}).get("testSet", {}).get("jira", {}).get("key", "")

    @staticmethod
    def get_test_issue_ids_from_gettestset_result(*, result):
        return [
            test.get("issueId", "") for test in result.get("getTestSet", {}).get("tests", {}).get("results", [])
        ]
    ...
    def display_testset_tests(*, response):
        ...
        for test_result in response.get("getTestSet", {}).get("tests", {}).get("results", []):
            jira_key = test_result.get("jira", {}).get("key", "")
            jira_summary = test_result.get("jira", {}).get("summary", "")
    ...
    def get_tests_in_testset(self, *, testset_id: str):
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_tests_in_testset.graphql"))
        variables = dict(
    ...
    def add_tests_to_testset(self, *, testset_id: str,  test_ids: list):
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "add_tests_to_testset.graphql"))
        variables = dict(
            testset_id=testset_id,
            test_ids=test_ids
        )
    ...
    def remove_tests_from_testset(self, *, testset_id: str,  test_ids: list):
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "remove_tests_from_testset.graphql"))
        variables = dict(
            testset_id=testset_id,
            test_ids=test_ids
        )
    ...
    def create_testset_with_tests(self, *, summary: str,  project_key: str, test_ids: list):
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_testset_with_tests.graphql"))
        variables = dict(
            summary=summary,
        ...
        log.debug(f"Created TestSet {testset_jira_id} [{testset_id}] with Tests {test_ids}")
        return response

    def create_empty_testset(self, *, summary: str,  project_key: str):
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "create_empty_testset.graphql"))
        variables = dict(
            summary=summary,
            project_key=project_key
        )
    ...
    def get_testsets_by_jql(self, jql: str) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_testsets_by_jql.graphql")
        )
...
class GQLTestStatus(MetaCategory):
    ...
    def get_teststatuses(self) -> dict:
        ...
        """
        query = load_query_from_file(
            os.path.join(THIS_DIR, GQL_QUERY_DIR, self.template_dir, "get_test_statuses.graphql")
        )
