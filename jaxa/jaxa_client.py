...
from .api.xray import TransportFactory, AsyncTransport, Transport, XRayRESTAPI, XRayGraphQLAPI
...
class JAXAClient:
    ...
    def _init_jira_rest_api_client(self, jira_url: Optional[str] = None, **kwargs):
        ...
        self._jira = JiraRESTAPI(url=jira_url or config("JAXA_JIRA_REST_BASEURL"), **kwargs)
    ...
    def _init_xray_rest_api_client(self, rest_url: Optional[str] = None, **kwargs):
        ...
        self._rest = XRayRESTAPI(url=rest_url or config("JAXA_XRAY_REST_BASEURL"), **kwargs)
    ...
    def _init_xray_graphql_qpi_client(
        ...
        self._gql_headers.update(self.gql_auth_header(client_id=client_id, client_secret=client_secret))

        if transport is None:
            self._transport = TransportFactory.get_transport_class(DEFAULT_TRANSPORT_LAYER)(
                url=config("JAXA_XRAY_GQL_BASEURL"),
    ...
    def gql_auth_header(self, client_id: Optional[str] = None, client_secret: Optional[str] = None):
